...
int main(int argc, char **argv)
{
  ...
  for (c = 1; c < argc; c++)
  {
    ...
    else
    {
      ...
      strcpy(dumpfile,sidname);
      replace(dumpfile,"dmp");
    }
  }
  ...
  if ((argc < 2) || (usage))
  {
    fprintf(stderr, "Usage: SIDDUMP <sidfile> [options]\n"
		   ...
		   "-b<file>  Output register dump in binary format suitable for .sidplay on TBBlue\n");
    return 1;
  }
  ...
  if (binary_output)
  {
  	fdmp = fopen (dumpfile, "wb");
  	if (!fdmp)
  	{
  		fprintf (stderr, "Error: dumpfile cannot be created.\n");
  		return 1;
  	}
  }
  ...
  { // CPU cycles, Raster lines, Raster lines with badlines on every 8th line, first line included
   ...
   if (!binary_output) printf("\n");

  if (!csv_output && !binary_output) {
    printf("+-------+---------------------------+---------------------------+---------------------------+---------------+");
    if (profiling)
    {
      printf("------------+");
